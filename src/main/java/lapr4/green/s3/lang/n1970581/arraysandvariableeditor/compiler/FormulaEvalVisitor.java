package lapr4.green.s3.lang.n1970581.arraysandvariableeditor.compiler;
...
import lapr4.green.s3.lang.n1970581.formula.compiler.Formula4BaseVisitor;
import lapr4.green.s3.lang.n1970581.formula.compiler.Formula4Parser;
 ...
 */
public class FormulaEvalVisitor extends Formula4BaseVisitor<Expression> {
    ...
    @Override
    public Expression visitExpression(Formula4Parser.ExpressionContext ctx) {
//        return visit(ctx.comparison());
    ...
    @Override
    public Expression visitComparison(Formula4Parser.ComparisonContext ctx) {
    ...
    @Override
    public Expression visitConcatenation(Formula4Parser.ConcatenationContext ctx) {
        try {
...
//                if (ctx.getChild(1).getChildCount() == 0) { // Conclude that operator is on the right
    ...
    @Override
    public Expression visitAtom(Formula4Parser.AtomContext ctx) {
        if (ctx.getChildCount() == 3) {
            return visit(ctx.getChild(1));
        }
    ...
    @Override
    public Expression visitFunction_call(Formula4Parser.Function_callContext ctx) {
    ...
    @Override
    public Expression visitReference(Formula4Parser.ReferenceContext ctx) {
        try {
            if (ctx.getChildCount() == 3) {
                BinaryOperator operator = Language.getInstance().getBinaryOperator(ctx.getChild(1).getText());
    ...
    @Override
    public Expression visitLiteral(Formula4Parser.LiteralContext ctx) {
        Token t = (Token) ctx.getChild(0).getPayload();

        if (t.getType() == Formula4Parser.NUMBER) {
            return new Literal(Value.parseValue(ctx.getText()));
        } else if (t.getType() == Formula4Parser.STRING) {
    ...
    @Override
    public Expression visitAssignment(Formula4Parser.AssignmentContext ctx) {
        if (ctx.ASSIGN() != null) {
            try {
                // Convert binary operation
    ...
    @Override
    public Expression visitBlock(Formula4Parser.BlockContext ctx) {
